<?php
  ...
  namespace Simplon\Vendor\Mustache;
  ...
  class Mustache
  {
    ...
    {
      return sprintf('/%s%%\\s*(?P<pragma_name>[\\w_-]+)(?P<options_string>(?: [\\w]+=[\\w]+)*)\\s*%s\\n?/s', preg_quote($otag, '/'), preg_quote($ctag, '/'));
    }
    ...
    protected function _renderPragmas($template)
    {
      ...
      return preg_replace_callback($regEx, array(
        $this,
        '_renderPragma'
      ), $template);
    }
